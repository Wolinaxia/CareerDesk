...
源码安装和桌面发行包提供 `careerdesk-resume-mcp`，可让 Codex 等本地 MCP 客户端读取岗位 JD、读写简历正文。它复用 CareerDesk 配置的本地用户和数据目录，**仅允许本机 stdio 使用，禁止改成 SSE/HTTP 或暴露到网络**。简历名称唯一，新建时绝不覆盖同名简历；替换正文必须带上读取时返回的 `expected_content_hash`，内容已变更则直接失败。写入操作不可安全重试，失败或超时后必须先重新读取简历列表与岗位绑定再重试。

源码环境可将下面的命令配置为 MCP Server：

```text
<仓库路径>/backend/.venv/bin/python -m careerdesk.mcp.resume_server
```

`save_resume` 归档本地源文件时仅能读取 `APP_RESUME_MCP_ARCHIVE_SOURCE_ROOTS` 允许的目录，默认是 `~/Desktop`、`~/Documents`、`~/Downloads`。

共 7 个工具：`list_applications`、`get_application_jd`、`list_resumes`、`get_resume_text`、`get_application_resume` 用于读取岗位与简历，其中 JD 全文不截断；`save_resume` 新建简历并可归档源文件；`update_resume_text` 按内容哈希替换正文。新建简历的标注状态为待处理，需要在 CareerDesk 界面里触发简历标注。桌面发行包中请使用随包附带的 `careerdesk-resume-mcp` 可执行文件。
