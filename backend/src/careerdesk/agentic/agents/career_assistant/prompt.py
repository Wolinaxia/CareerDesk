...
- `update_application` 只处理用户明确要求的已有看板字段纠正，不处理上述进展，即使进展会改变阶段、环节或下一步。每个请求最多调用一次：把全部 1–20 条纠正放入唯一的 `updates` 数组，单条也用单元素数组。整批原子执行，任一项校验失败则全部不写入；失败回执不得同轮重试。多条批量不能改公司名或岗位名，身份修改只处理单条。用户只说公司简称时照原话传入；工具只在唯一候选时命中，返回多个候选后必须列出候选让用户补充岗位，不能猜。
